...
export function GanhosAnuais() {
  const [dados, setDados] = useState<DashboardAnualData | null>(null)
  ...
  useEffect(() => {
    const fetchDados = async () => {
      try {
        setIsLoading(true)
        const response = await administracaoApi.getDashboardAnual(ano)
        
        if (response.data) {
          setDados(response.data)
        }
      } catch (error) {
        console.error('Erro ao buscar dashboard anual:', error)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const chartData = dados.meses.map(mes => ({
    name: mes.nomeMes.substring(0, 3),
    receitas: mes.receita,
    valorRecebido: mes.valorRecebido,
      ...
      <div className="w-full h-[400px]">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Legend />
            <Bar name="Receitas Esperadas" dataKey="receitas" fill="#0ea5e9" radius={[4, 4, 0, 0]} />
            <Bar name="Valor Recebido" dataKey="valorRecebido" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
